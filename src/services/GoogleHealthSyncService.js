...
class GoogleHealthSyncService {
  ...
  static groupDataByDate(hrData = [], stepsData = [], sleepData = [], oxygenData = []) {
    ...
    (sleepData || []).forEach(item => {
      // Use startTime for grouping because sleep might span multiple days
      const dateStr = getDateString(item.startTime);
    ...
    (oxygenData || []).forEach(item => {
      const dateStr = getDateString(item.time);
  ...
  static processStepsData(fitnessRecord, stepsData) {
    ...
    stepsData.forEach(item => {
      ...
      const estimatedDistance = item.count * 0.762;
      ...
      if (existingTimeRanges.has(timeRangeKey)) {
        ...
        fitnessRecord.steps.stepsByHour[index].steps = item.count;
  ...
  static processSleepData(fitnessRecord, sleepData) {
    ...
    sleepData.forEach(item => {
      ...
      if (existingTimeRanges.has(timeRangeKey)) {
        ...
        fitnessRecord.sleep.sessions[index].name = item.title || 'Sleep Session';
        fitnessRecord.sleep.sessions[index].description = item.notes || '';
      } else {
        // Add new session
  ...
  static processOxygenData(fitnessRecord, oxygenData) {
    ...
    if (allOxygenReadings.length > 0) {
      ...
      fitnessRecord.vitals.oxygenSaturation = sortedOxygenData[0].percentage;
    }
  }
   ...
   */
  static async syncGoogleFitData(userId, hr, steps, sleep, oxygen) {
    try {
      console.log('Starting Google Fit sync with data:', {
...
module.exports = GoogleHealthSyncService;